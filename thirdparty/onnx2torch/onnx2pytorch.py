...
def rebuild_lrn(node, weights):
    ...
    for att in node.attribute:
        kwargs[rebuild_lrn.lrn_attr_map[att.name]] = (
            att.f if att.name != "size" else att.i
        )
...
def rebuild_conv(node, weights):
    ...
    kwargs = {}
    for att in node.attribute:
        kwargs[rebuild_conv.conv_attr_map[att.name]] = (
            list(att.ints) if att.name != "group" else att.i
        )
...
def rebuild_batchnormalization(node, weights):
    rebuild_batchnormalization.bn_attr_map = {"epsilon": "eps", "momentum": "momentum"}
...
def rebuild_prelu(node, weights):
    return nn.PReLU(), node.input, node.output
