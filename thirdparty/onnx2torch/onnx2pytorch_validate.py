...
def construct_mxnext_model(onnx_file, test_input):
    ...
    mod = mx.mod.Module(
        symbol=sym, data_names=data_names, context=mx.cpu(), label_names=None
    )
    print(sym)
    # exit(0)
    mod.bind(
        for_training=False,
        data_shapes=[(data_names[0], test_input.shape)],
        label_shapes=None,
    )
...
def main():
    ok_onnx_model_files = [
        ...
        "/mnt/DATA2/congvm/Workspace/ABAW2/ckpts/models/buffalo_l/w600k_r50.onnx"
    ]
