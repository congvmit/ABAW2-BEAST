...
from . import alexnet_dnn_block as alexnet_dnn
from . import alexnet_mcdo_block as alexnet_mcdo
from . import smoothing_block as smoothing
from . import classifier_block as classifier
...
class AlexNet(nn.Module):
    def __init__(
        ...
        **block_kwargs
        ...
        if stem:
            self.layer0.append(
                block(3, 64, kernel_size=11, stride=4, padding=2, **block_kwargs)
            )
        else:
            self.layer0.append(
                block(3, 64, kernel_size=3, stride=2, padding=1, **block_kwargs)
            )
        ...
        self.layer1.append(
            block(64, 192, kernel_size=(5, 5), padding=2, **block_kwargs)
        )
        ...
        self.smooth0 = self._make_smooth_layer(
            sblock, 64, num_sblocks[0], **block_kwargs
        )
        self.smooth1 = self._make_smooth_layer(
            sblock, 192, num_sblocks[1], **block_kwargs
        )
        self.smooth2 = self._make_smooth_layer(
            sblock, 256, num_sblocks[2], **block_kwargs
        )
        ...
        if cblock is classifier.MLPBlock:
            ...
            self.classifier.append(
                cblock(out_size * out_size * 256, num_classes, **block_kwargs)
            )
        else:
...
def mcdo_smooth(
    num_classes=10, stem=True, name="alexnet_mcdo_smoothing", **block_kwargs
):
    return AlexNet(
        alexnet_mcdo.BasicBlock,
        num_sblocks=[1, 1, 1],
