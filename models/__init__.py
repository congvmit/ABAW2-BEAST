...
def get_model(name, num_classes=10, stem=False, verbose=True, **block_kwargs):
    ...
    if name in ["alexnet_dnn"]:
        model = alexnet.dnn(num_classes=num_classes,
                            stem=stem, name=name, **block_kwargs)
    elif name in ["alexnet_mcdo"]:
        model = alexnet.mcdo(num_classes=num_classes,
                             stem=stem, name=name, **block_kwargs)
    elif name in ["alexnet_dnn_smoothing"]:
        model = alexnet.dnn_smooth(
            num_classes=num_classes, stem=stem, name=name, **block_kwargs)
    elif name in ["alexnet_mcdo_smoothing"]:
        model = alexnet.mcdo_smooth(
            num_classes=num_classes, stem=stem, name=name, **block_kwargs)
    # VGG
        ...
        model = vgg.dnn_smooth_13(
            num_classes=num_classes, name=name, **block_kwargs)
    elif name in ["vgg_mcdo_smoothing_13"]:
        model = vgg.mcdo_smooth_13(
            num_classes=num_classes, name=name, **block_kwargs)
        ...
        model = vgg.dnn_smooth_16(
            num_classes=num_classes, name=name, **block_kwargs)
    elif name in ["vgg_mcdo_smoothing_16"]:
        model = vgg.mcdo_smooth_16(
            num_classes=num_classes, name=name, **block_kwargs)
...
def save_snapshot(model, dataset_name, uid, typ, optimizer=None, root="models_checkpoints"):
    snapshot_path = os.path.join(
        root, dataset_name, model.name, "%s_%s_%s" % (dataset_name, model.name, uid))
...
def load(model, dataset_name, uid, optimizer=None, root="models_checkpoints"):
    ...
    save_path = os.path.join(
        checkpoint_path, "%s_%s_%s.pth.tar" % (dataset_name, model.name, uid))
...
def stats(model, xs=None):
    stat_str = "model: %s , params: %.1fM" % (
        model.name, count_parameters(model) / 1e6)
    if xs is not None:
        model.eval()
        ys = model(xs)
        ...
        if not isinstance(ys, tuple):
            ...
            for i, y_ in enumerate(ys):
                stat_str += ", output %i: %s" % (i, list(y_.size()))
    print(stat_str)
